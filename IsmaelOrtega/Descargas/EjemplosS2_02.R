...
moda <- function(vector){
  ...
  pos.max <- which(f.abs == max.f.abs) # posición(es) de la(s) máxima(s) frecuencia(s) absoluta(s)     ## Which recupera la posicion de un valor
  print("La(s) moda(s) es(son): ")
  print(names(f.abs[pos.max]))        ## Obtiene los numeros(encabezado) de la moda 
  paste("Con una frecuencia de: ", unique(f.abs[pos.max]))       ##Obtiene el numero de repeticiones de la moda.
}
...
table(x) # obtenemos las frecuencias absolutas de los valores de la muestra       #Es una tabla de frecuencias
